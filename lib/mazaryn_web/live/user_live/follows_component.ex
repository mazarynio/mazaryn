defmodule MazarynWeb.UserLive.FollowsComponent do
    ...
    <div id="follows">
      ...
      <%= if @search_query != "" and @search_result == :user_not_exist do %>
        <div class="text-center py-8 text-gray-500">
          ...
          <p class="text-lg">No following found with username "<%= @search_query %>"</p>
        </div>
      <% else %>
        <%= for user <- @displayed_following do %>
      ...
      <%= if @search_query == "" do %>
        <div class="text-sm text-gray-500 mt-4">
          Total following: <%= length(@following) %>
        </div>
      <% else %>
  ...
  def update(%{current_user: %Account.User{following: following_ids} = current_user} = assigns, socket) do
    following = get_following(following_ids)

    {:ok,
     socket
     |> assign(assigns)
     |> assign(following: following)
     |> assign(displayed_following: following)
     |> assign(search_query: "")
     |> assign(search_result: nil)
     |> assign(current_user_id: get_user_id(current_user))}
  end

  def handle_event("search_followings", %{"value" => query}, socket) do
    ...
    if query == "" do
      {:noreply,
       ...
       |> assign(displayed_following: socket.assigns.following)
       |> assign(search_result: nil)}
    else
      case Core.UserClient.search_followings(socket.assigns.current_user_id, query) do
        :user_not_exist ->
          {:noreply,
           socket
           |> assign(search_query: query)
           |> assign(displayed_following: [])
        ...
        user_tuple ->
          ...
          {:noreply,
           socket
           |> assign(search_query: query)
           |> assign(displayed_following: [user])
