defmodule MazarynWeb.HomeLive.CommentHandler do
  ...
  def handle_delete_reply(%{"reply-id" => reply_id, "comment-id" => comment_id}, comments) do
    Logger.info(
      "Starting enhanced reply deletion for reply #{reply_id} from comment #{comment_id}"
    )
  ...
  def handle_validate_update_comment(%{"comment" => comment_params}) do
    changeset =
      Comment.update_changeset(%Comment{}, comment_params)
      |> Map.put(:action, :validate)
  ...
  defp spawn_enhanced_reply_deletion_task(reply_id, comment_id, reply_to_delete) do
    ...
    spawn(fn ->
      try do
        ...
        results =
          Enum.map(deletion_attempts, fn attempt ->
            try do
              case attempt.() do
                :ok -> {:ok, :deleted}
                {:ok, _} -> {:ok, :deleted}
        ...
        if success_count > 0 do
          Logger.info(
            "Enhanced reply deletion succeeded for #{reply_id} (#{success_count} methods)"
          )
          ...
          Logger.error(
            "All enhanced reply deletion methods failed for #{reply_id}: #{inspect(results)}"
          )
  ...
  defp attempt_comment_reply_cleanup(comment_id, reply_id) do
    ...
    case :postdb.remove_reply_from_comment(comment_id_charlist, reply_id_charlist) do
      ...
      error ->
        ...
        filtered_replies =
          Enum.reject(current_replies, fn reply ->
            case reply do
  ...
  defp add_reply_to_comment(comments, comment_id, new_reply) do
    Enum.map(comments, fn comment ->
      if to_string(comment.id) == to_string(comment_id) do
        ...
        Logger.info(
          "Added reply to comment #{comment_id}. Total replies: #{length(updated_replies)}"
        )
  ...
  defp remove_reply_from_comments(comments, comment_id, reply_id) do
    ...
    Enum.map(comments, fn comment ->
      if to_string(comment.id) == to_string(comment_id) do
        updated_replies =
          Enum.reject(comment.replies || [], fn reply ->
  ...
  defp save_comment_with_immediate_content(params, changeset) do
    ...
    optimistic_comment =
      create_comment_with_real_content(params.post_id, params.author, params.content)
    ...
    post =
      case CommentUtilities.rebuild_post_safe(params.post_id) do
        {:ok, post} ->
          post
        ...
        case result do
          {:ok, real_comment} when is_map(real_comment) ->
            if Map.has_key?(real_comment, :id) and real_comment.id do
              ...
              Logger.info(
                "Background save succeeded: #{optimistic_comment.id} -> #{real_comment.id}"
              )
            end
        ...
        result =
          try do
            case Posts.create_comment(changeset) do
              {:ok, comment} when is_map(comment) -> {:ok, comment}
              %{} = comment when map_size(comment) > 0 -> {:ok, comment}
              %{} -> {:error, :empty_result}
              {:error, reason} -> {:error, reason}
              other -> {:error, {:unexpected_result, other}}
            end
  ...
  defp spawn_background_update_attempt(comment_id, new_content) do
    spawn(fn ->
      try do
        comment_id_charlist =
          if is_binary(comment_id), do: to_charlist(comment_id), else: comment_id
