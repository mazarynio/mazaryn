defmodule MazarynWeb.HomeLive.Home do
  ...
  def mount(_params, %{"session_uuid" => session_uuid} = _session, socket) do
    ...
    result = with {:ok, user} <- get_user_by_session_cached(session_uuid) do
      ...
      |> assign_optimistic_posts()

      schedule_async_operations_home(user.id)
      {:ok, socket}
    else
      {:error, reason} ->
        ...
        schedule_async_operations_home(user_id)
        {:ok, socket}
    end
  ...
  def handle_info({:load_user_and_posts_async, user_id}, socket) do
    ...
    socket = if user_result do
      ...
      socket |> assign(posts: posts_result, posts_loading: false)
    end
  ...
  defp schedule_async_operations_home(user_id) do
    if user_id do
  ...
  defp assign_optimistic_posts(socket) do
    user = socket.assigns[:user]
    cached_posts = if user, do: get_cached_posts_fallback(user.id), else: []
  ...
  defp track_performance(operation, start_time) do
    ...
    case duration do
      d when d > 1000 -> Logger.warn("⚠️ Slow operation #{operation}: #{d}ms")
      d when d > 500 -> Logger.info("⏰ Operation #{operation}: #{d}ms")
      _ -> Logger.debug("✅ Operation #{operation} completed quickly")
    end
