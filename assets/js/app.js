...
let Hooks = {
  ...
  EmojiHandler: {
    ...
    initEmojiHandler() {
      ...
      if (this.inputHandler) postContent?.removeEventListener("input", this.inputHandler)
      ...
      if (postContent) {
        this.inputHandler = () => this.updateCharacterCount(postContent, charCount, charProgress)
  ...
  Notifications: {
    mounted() {
      ...
      this.handleEvent("new_notification", () => this.el.classList.remove("hidden"))
    },
    updated() {
      this.updateVisibility()
    },
    updateVisibility() {
      this.el.classList.toggle("hidden", window.location.pathname === "/en/notifications")
    }
  },

  ChatNotifications: {
    mounted() {
      this.badgeEl = this.el.querySelector(".chat-notification-badge") || this.createBadge()
