...
handle_call({remote_pin_post, PostID, Options}, From, State) when is_map(Options) ->
    handle_call({remote_pin_post, PostID, undefined, Options}, From, State);

handle_call({remote_pin_post, PostID, Service, Options}, _From, State) ->
    ...
    case check_rate_limits(get_requester_info(Options)) of
        {ok, _} ->
            {Pid, Ref} = spawn_monitor(fun() ->
                ...
                gen_server:cast(?MODULE, {pin_complete, PostID, RequestId, Result, StartTime})
            end),
            
            NewActivePins = maps:put({PostID, RequestId}, {Pid, Ref, Options}, State#state.active_pins),
            {reply, {ok, #{request_id => RequestId}}, State#state{active_pins = NewActivePins}};
        {error, RateLimitReason} = Error ->
            logger:warning("[~s] Rate limit exceeded: ~p", [RequestId, RateLimitReason]),
            {reply, Error, State}
    end;

handle_call({verify_pin_health, PinID}, _From, State) ->
    Result = verify_pin_health(PinID),
    {reply, Result, State};

handle_call({get_pin_info, PinID}, _From, State) ->
    Result = get_pin_info(PinID),
    {reply, Result, State};

handle_call({unpin_post, PostID, Options}, _From, State) ->
    Result = unpin_post(PostID, Options),
    {reply, Result, State};

handle_call({get_gateway_url, CID, Options}, _From, State) ->
    Result = get_gateway_url(CID, Options),
    {reply, Result, State};

handle_call({get_pin_statistics, Filters}, _From, State) ->
    Result = get_pin_statistics(Filters),
    {reply, Result, State}.

%% @doc Handle asynchronous casts
handle_cast({bulk_pin_posts, PostIDs, Options}, State) when is_list(PostIDs), length(PostIDs) =< ?MAX_BULK_PINS ->
    BatchID = generate_batch_id(),
    {Pid, Ref} = spawn_monitor(fun() ->
        Results = lists:map(
            fun(PostID) ->
                try
                    case pin_post:remote_pin_post(PostID, Options#{batch_id => BatchID}) of
    ...
    NewBatchJobs = maps:put(BatchID, {Pid, Ref}, State#state.batch_jobs),
    {noreply, State#state{batch_jobs = NewBatchJobs}};

handle_cast({bulk_pin_posts, _, _}, State) ->
    {noreply, State};

handle_cast({schedule_pin_operation, PostID, ScheduleTime, Options}, State) ->
    ...
    NewScheduledJobs = maps:put(JobID, Job, State#state.scheduled_jobs),
    {noreply, State#state{scheduled_jobs = NewScheduledJobs}};

handle_cast({update_pinning_metrics, PostID, Author, ElapsedTime}, State) ->
    spawn(fun() ->
        update_pinning_metrics(PostID, Author, ElapsedTime)
    end),
    {noreply, State};

handle_cast({pin_complete, PostID, RequestId, Result, StartTime}, State) ->
    {_Pid, _Ref, Options} = maps:get({PostID, RequestId}, State#state.active_pins, {undefined, undefined, #{}}),
    ...
    NewActivePins = maps:remove({PostID, RequestId}, State#state.active_pins),
    {noreply, State#state{active_pins = NewActivePins}}.

%% @doc Handle info messages
handle_info({'DOWN', Ref, process, Pid, Reason}, State) ->
    case find_active_pin(Pid, Ref, State#state.active_pins) of
        {{PostID, RequestId}, _} ->
            logger:error("Pinning process for post ~p (request ~p) failed: ~p", [PostID, RequestId, Reason]),
            NewActivePins = maps:remove({PostID, RequestId}, State#state.active_pins),
            {noreply, State#state{active_pins = NewActivePins}};
        not_found ->
            case find_batch_job(Pid, Ref, State#state.batch_jobs) of
                {BatchID, _} ->
                    logger:error("Batch job ~p failed: ~p", [BatchID, Reason]),
                    NewBatchJobs = maps:remove(BatchID, State#state.batch_jobs),
                    {noreply, State#state{batch_jobs = NewBatchJobs}};
                not_found ->
                    {noreply, State}
            end
...
execute_pin_transaction(PostID, Service, Options, RequestId) ->
        ...
        case mnesia:read({post, PostID}) of
            [Post] ->
                ...
                case storage_quota:check_storage_quota(UserID, BusinessID) of
                    {ok, Quota} when is_record(Quota, storage_quota) ->
                        logger:debug("[~s] Quota: ~p", [RequestId, Quota]),
                        ...
                        {ok, #{request_id := ContentReqId, ref := ContentRef}} = storage_quota:calculate_content_size(ContentCID),
                        {ok, #{request_id := MediaReqId, ref := MediaRef}} = storage_quota:calculate_media_size(MediaCIDs),
                        ContentSize = receive_size_result(ContentReqId, ContentRef, content_size_result),
                        MediaSize = receive_size_result(MediaReqId, MediaRef, media_size_result),
                        TotalSize = ContentSize + MediaSize,
                        EncryptedContent = case maps:get(encrypt, Options, false) of
                            true -> encrypt_content(ContentCID, PinID, Options);
                            false -> undefined
                        end,
                        ...
                        case storage_quota:update_storage_quota(Quota, TotalSize) of
                            ...
                            QuotaError ->
                                spawn(fun() -> rollback_pin_operation(ContentCID, MediaCIDs, ClusterService) end),
                    ...
                    Invalid ->
                        logger:error("[~s] Invalid quota response: ~p", [RequestId, Invalid]),
                        {error, {invalid_quota_response, Invalid}}
                end;
            [] ->
    ...
    logger:debug("[~s] Starting Mnesia transaction for post ~p", [RequestId, PostID]),
...
receive_size_result(RequestId, Ref, MessageType) ->
    receive
        {MessageType, RequestId, Ref, Size} when is_integer(Size), Size >= 0 ->
            logger:debug("Received ~p for request ~p: ~p bytes", [MessageType, RequestId, Size]),
            Size;
        {MessageType, RequestId, Ref, InvalidSize} ->
            logger:warning("Invalid size ~p received for request ~p, using 0", [InvalidSize, RequestId]),
            0
    after 60000 ->
        logger:error("Timeout waiting for ~p for request ~p", [MessageType, RequestId]),
        0
    end.
...
create_pin_info_record(PostID, PinID, UserID, PinParams, ContentCID, MediaCIDs, Service, TotalSize, Metadata, EncryptedContent) ->
    logger:info("Creating pin ~p for post ~p: total_size=~p bytes", [PinID, PostID, TotalSize]),
    Size = case TotalSize < 0 of
        true ->
...
%% @doc Ensure replication for pinned content
...
%% @doc Find active pin by Pid and Ref
find_active_pin(Pid, Ref, ActivePins) ->
    Filtered = maps:filter(fun(_, {P, R, _}) -> P == Pid andalso R == Ref end, ActivePins),
    case maps:to_list(Filtered) of
        [{Key, Value}] -> {Key, Value};
        [] -> not_found
    end.

%% @doc Find batch job by Pid and Ref
find_batch_job(Pid, Ref, BatchJobs) ->
    Filtered = maps:filter(fun(_, {P, R}) -> P == Pid andalso R == Ref end, BatchJobs),
    case maps:to_list(Filtered) of
        [{Key, Value}] -> {Key, Value};
        [] -> not_found
    end.

%% @doc Generate a unique batch ID for bulk operations
generate_batch_id() ->
