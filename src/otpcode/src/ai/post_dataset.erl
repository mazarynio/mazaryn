...
-export([serialize_post/1, save_to_tfrecord/2, save_to_csv/2, save_to_json/2, save_to_parquet/2, append_to_tfrecord/2, append_to_csv/2, append_to_json/2]).
...
append_to_tfrecord(Post, Filename) ->
    case application:ensure_all_started(gun) of
        {ok, _} ->
            JsonData = serialize_post(Post),
            JsonDataWithFilename = JsonData#{<<"filename">> => list_to_binary(Filename)}, 
            ...
            Headers = [{<<"content-type">>, <<"application/json">>}],
            StreamRef = gun:post(ConnPid, "/api/append_post_to_tfrecord", Headers, jsone:encode(JsonDataWithFilename)),
            ...
            case gun:await(ConnPid, StreamRef) of
                {response, nofin, 200, _RespHeaders} ->
                    ...
                    case file:write_file(Filename, RespBody, [append]) of
                        ok ->
                            io:format("Post appended to ~s~n", [Filename]),
                            ok;
                        {error, Reason} ->
                            io:format("Error writing to file ~s: ~p~n", [Filename, Reason]),
                            {error, Reason}
...
append_to_csv(Post, Filename) ->
    case application:ensure_all_started(gun) of
        {ok, _} ->
            ...
            Headers = [{<<"content-type">>, <<"application/json">>}],
            StreamRef = gun:post(ConnPid, "/api/append_post_to_csv", Headers, jsone:encode(JsonDataWithFilename)),
...
append_to_json(Post, Filename) ->
    case application:ensure_all_started(gun) of
        {ok, _} ->
            ...
            Headers = [{<<"content-type">>, <<"application/json">>}],
            StreamRef = gun:post(ConnPid, "/api/append_post_to_json", Headers, jsone:encode(JsonDataWithFilename)),
